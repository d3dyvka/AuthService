...
from src.application.dto import TokenPairDTO, VerifyLoginDTO
...
class VerifyLoginUseCase:
    ...
    async def execute(self, dto: VerifyLoginDTO) -> TokenPairDTO:
        user = await self._user_repo.get_by_email(dto.email)
        if user is None:
            raise UserNotFoundException()

        attempts = await self._code_repo.get_attempts(dto.email, "login")
        if attempts >= MAX_ATTEMPTS:
            await self._code_repo.delete_code(dto.email, "login")
            raise TooManyAttemptsException()

        stored_code = await self._code_repo.get_code(dto.email, "login")
        if stored_code is None or stored_code != dto.code:
            await self._code_repo.increment_attempts(dto.email, "login")
        ...
        logger.info("User login verified (id=%s)", user.id)
